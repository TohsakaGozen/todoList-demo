<template>
  <transition name="todo" appear>
    <div class="box1">
      <div>
        <input class="cb" type="checkbox" v-model="todo.done">
        <span v-show="!todo.isEdit">{{ todo.title }}</span>
        <input ref="input" id="edit" type="text" v-show="todo.isEdit" v-model="todo.title" @keyup.enter="Fset"
          @blur="Fset">
      </div>
      <div>
...
.todo-enter-active {
  animation: Sakura 0.8s ;
}

.todo-leave-active {
  animation: Sakura 0.8s  reverse;
}
